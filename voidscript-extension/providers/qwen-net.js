...
// that carry VoidScript commands) in a Monaco editor that DISPOSES off-screen
...
// done flag into a DOM node (#vs-qwen-net, JSON textContent) that the isolated
...
(() => {
  "use strict";
  if (window.__vsQwenNet) return;
  window.__vsQwenNet = true;

  const NODE_ID = "vs-qwen-net";
  const node = () => {
