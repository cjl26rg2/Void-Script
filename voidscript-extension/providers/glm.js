...
const VSProvider = (() => {
  ...
  const S = {
    ...
    // so "voidscript_….jpg JPG 179 KB Output of 'screen_capture'…" failed to match
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = S.thinking + ", .vs-chip, " + S.attachment + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  // reconciles the composer subtree, so we must NOT insert our #vs-bar into it
  ...
  // bar in #vs-root, positions it to hug that card's top edge at full width, and
  ...
  // identity) with .vs-cmd-mask so a CSS rule re-hides recreated wrappers with
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const cc of item.querySelectorAll(S.copyCodeBtn)) {
      ...
      if (!wrap || wrap.closest(".vs-chip")) continue;
    ...
    [...reply.children].forEach((el) => {
      if (el.classList.contains("vs-chip") || el.closest(S.thinking) ||
