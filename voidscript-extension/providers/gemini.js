...
const VSProvider = (() => {
  ...
  // VoidScript bar stuck on "Agent active" with no way to start a new session.
  ...
  function setInputLock(on) {
    ...
    if (on) ph.setAttribute("data-vs-locked", "1");
    else ph.removeAttribute("data-vs-locked");
  }
  ...
  function truncateForSend(text) {
    ...
    const marker = (what) =>
      `\n\n[…VoidScript: result truncated (${what}) so it can be pasted into ` +
  ...
  // function's real job is applying the .vs-tool-hide classes correctly.
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    for (const mc of replies) {
      ...
      mc.querySelectorAll(S.codeWrap).forEach((cb) => {
        if (cb.closest(".vs-chip")) return;
        if (CMD_SHAPE.test(cb.textContent || "")) {
          cb.classList.add("vs-tool-hide");
          ...
          // rule `message-content.vs-cmd-mask code-block` keeps every
          // recreated block hidden with zero flash.
          mc.classList.add("vs-cmd-mask");
