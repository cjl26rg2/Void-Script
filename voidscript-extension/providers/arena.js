...
// Exports the same VSProvider interface as providers/deepseek.js; the core
...
const VSProvider = (() => {
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = ".vs-chip" + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  // insert #vs-bar into it (the diff could nest the composer inside the bar, as
  // seen on Kimi/GLM). barAnchor() returns the rounded composer CARD; the core
  // (placeBar anchored branch) keeps the bar in #vs-root, hugs the card's top
  ...
  function streamText(item) {
    if (!item) return "";
    const md = proseOf(item);
    return md ? textWithout(md, ".vs-chip") : "";
  }
  ...
  function readAssistant() {
    ...
    return {
      present: true,
      reply: md ? textWithout(md, ".vs-chip").trim() : "",
      thinking: "",
      item,
    };
  }
  ...
  async function typeAndSend(text, images) {
    ...
    diag("arena.tas.enter", {
      textLen: (text || "").length,
      imgId: images ? images.__vsId : null,
      imgCount: images ? images.length : 0,
      attachedId: _attachedImages ? _attachedImages.__vsId : null,
    ...
    if (images && images.length && images !== _attachedImages) {
      ...
      try {
        ...
        diag("arena.tas.attached", { imgId: images.__vsId, ok, pendingAfter: pendingCount() });
      ...
      diag("arena.tas.skipAttach", { reason: !images || !images.length ? "no-images" : "same-set", imgId: images ? images.__vsId : null });
    }
  ...
  // /agent to a BETA generic provider (see agentProvider() below) instead, so
  // this gate only ever applies to the Direct chat route. The mode dropdown's
  // trigger is a `button[role="combobox"]` whose visible text IS the current
  // mode name; we read it to gate Start. Detection fails OPEN (unknown → allow)
  // so a DOM reskin never wrongly blocks the default.
  ...
  // with no open modal never matches. Our own UI lives in #vs-root and is skipped.
  function overlayBlocking() {
    for (const d of document.querySelectorAll('[role="dialog"]')) {
      if (d.closest("#vs-root")) continue;
  ...
  function tagImages(images) {
    if (images && images.__vsId == null) {
      try { Object.defineProperty(images, "__vsId", { value: ++_imgSeq, enumerable: false }); } catch { images.__vsId = ++_imgSeq; }
    }
    return images;
  }
  ...
  // on the next send (wiping per-element .vs-tool-hide), so also mark the turn
  // (its identity survives - the chip is anchored at item level) with
  // .vs-cmd-mask; the overlay.css rule keeps recreated code wrappers hidden.
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    md.querySelectorAll(S.codeWrap).forEach((cw) => {
      if (cw.closest(".vs-chip")) return;
      if (CMD_SHAPE.test(cw.textContent || "")) {
        cw.classList.add("vs-tool-hide");
    ...
    [...md.children].forEach((el) => {
      if (el.classList.contains("vs-chip") || el.querySelector(S.codeWrap)) return;
      const t = el.textContent || "";
      if (t.length < 600 && CMD_SHAPE.test(t)) {
        el.classList.add("vs-tool-hide");
        item.classList.add("vs-cmd-mask");
  ...
  // ── Agent Mode (route /agent) ─────────────────────────────────────────────
  // Agent Mode is a SEPARATE app from the chat route (/text/*): a TipTap /
  // ProseMirror contenteditable composer (NO <form>) with its own turn/output
  // DOM and no shared <ol> message list - every DOM assumption of the Direct
  // provider above breaks there. A best-effort BETA provider is built on the
  // generic factory (VSGeneric, loaded from providers/_generic.js) with GUESSED
  // selectors. It will almost certainly need live tuning on arena.ai/agent; the
  // README "Beta sites" note applies. We only present it when the URL is on the
  // /agent route, so the tuned Direct provider is used everywhere else.
  const AGENT_SELECTORS = {
    // TipTap/ProseMirror render fenced code as a real <pre>; the visible
    // composer is a contenteditable element (scoped away from other editors).
    editor: '.ProseMirror, .tiptap, div[contenteditable="true"]',
    // Unknown whether /agent shows a reasoning subtree; disabling stripping
    // avoids accidentally hiding real model output.
    thinking: "",
    // Guessed turn markers (many chat UIs use these roles/testids). VERIFY on
    // a live /agent conversation - without a working assistant selector turn
    // reading fails and the loop cannot detect a reply.
    userItem:
      '[data-message-author-role="user"],[class*="user-message"],[data-testid*="user"]',
    assistantItem:
      '[data-message-author-role="assistant"],[class*="assistant-message"],[class*="bot-message"]',
    composer: "",
    sendBtn: 'button[type="submit"],button[aria-label*="send message" i],button[aria-label*="send" i]',
    stopBtn: 'button[aria-label*="stop generation" i],button[aria-label*="stop" i]',
    codeWrap: "pre",
  ...
  function agentProvider() {
    return VSGeneric({
      ...
      unstableWarning:
        "Arena Agent mode support is BETA (best-effort selectors, not " +
        "validated live). If it stalls, sends nothing, or never detects " +
        "completion, the /agent DOM changed - report it so a tuned provider " +
        "can be written.",
    });
  }
  ...
  // Route by URL: the /agent app is a different DOM, so hand it the BETA
  // generic provider; everywhere else (Direct /c/* /text/*) use the tuned
  // Direct provider above.
  return location.pathname.startsWith("/agent") ? agentProvider() : directProvider;
})();
