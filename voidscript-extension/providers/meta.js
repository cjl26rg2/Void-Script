...
//    the framing is thin; the FULL VoidScript system prompt (with the "commands
...
const VSProvider = (() => {
  ...
  function textWithout(root, excludeSel) {
    if (!root) return "";
    const skip = ".vs-chip, " + S.reasoning + (excludeSel ? ", " + excludeSel : "");
    let t = "";
    const walk = (n) => {
  ...
  const editorEls = () => [...document.querySelectorAll(S.input)].filter((e) => !e.closest("#vs-root"));
  const isTextareaEditor = (e) => !!e && e.tagName === "TEXTAREA";
  // getEditor() = the ON-SCREEN editor (the Lexical div). The core anchors the
  // "Agent is working…" cover and the .vs-typing mask to P.getEditor(), so it MUST
  ...
  // #vs-bar into it. barAnchor() returns the rounded composer card; the core
  // keeps the bar in #vs-root and hugs the card's top edge.
  function barAnchor() {
  ...
  // contains the editor) so the cover leaves the controls - and the VS bar above -
  ...
  function streamText(item) {
    const b = bodyOf(item);
    return b ? textWithout(b, ".vs-chip") : "";
  }
  ...
  function readAssistant() {
    ...
    const b = bodyOf(item);
    return { present: true, reply: b ? textWithout(b, ".vs-chip").trim() : "", thinking: "", item };
  }
  ...
  const fileInputEl = () => {
    for (const inp of document.querySelectorAll('input[type="file"]')) {
      if (!inp.closest("#vs-root")) return inp;
    }
    return null;
  };
  ...
  function tagImages(images) {
    if (images && images.__vsId == null) {
      try { Object.defineProperty(images, "__vsId", { value: ++_imgSeq, enumerable: false }); }
      catch { images.__vsId = ++_imgSeq; }
    }
    return images;
  }
  ...
  async function typeAndSend(text, images) {
    ...
    if (images && images.length && images !== _attachedImages) {
      ...
      try {
        ...
        diag("meta.tas.attached", { ok, imgId: images.__vsId });
  ...
  // (its identity survives) with .vs-cmd-mask; the overlay.css rule keeps recreated
  ...
  function findToolBlockSpot(item /*, chip */) {
    ...
    b.querySelectorAll("p, div").forEach((el) => {
      if (el.closest(".vs-chip, .vs-tool-hide, " + S.codeWrap)) return;
  ...
  return {
    ...
    // stay visible & usable - nor the VS bar anchored above. coverMaxH lifts the
    // core's 200px clamp so a grown (multi-line, up to the vs-typing 140px cap)
