...
// core (core/main.js) only ever talks to the VSProvider interface this file
...
const VSProvider = (() => {
  ...
  function streamText(item) {
    ...
    const replyTxt = [...item.querySelectorAll(S.markdown)]
      .filter((m) => !m.closest(S.thinking) && !m.closest(".vs-chip"))
      .map((m) => m.textContent)
      .join("");
    return thinkTxt + "\n" + replyTxt;
  }
    ...
    for (const container of containers) {
      ...
      while (i < kids.length) {
        ...
        for (let k = runStart; k <= runEnd; k++) {
          ...
          hide.classList.add("vs-tool-hide");
  ...
  return {
    ...
    init({ diag: d } = {}) {
      ...
      // confirmed from the page (read document.documentElement.dataset.vsDsVer).
      // BUMP DS_VER on meaningful deepseek.js changes worth verifying live.
      try { document.documentElement.setAttribute("data-vs-ds-ver", "2026-07_vision-badge-priority"); } catch {}
    },
    // turns
