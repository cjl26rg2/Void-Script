...
// Exports the same VSProvider interface as providers/deepseek.js and kimi.js;
...
const VSProvider = (() => {
  ...
  // MutationObserver snapshots its joined source into `pre.dataset.vsCode`,
  ...
  function snapshotCode(pre) {
    ...
    if (!live) return;
    const prev = pre.dataset.vsCode || "";
    if (live.length > prev.length) pre.dataset.vsCode = live;
  }
  ...
  function codeText(pre) {
    const cached = pre.dataset.vsCode || "";
    const live = codeLinesText(pre);
  ...
  // `#vs-qwen-net` node as JSON { rid, text, done, t }. netLatest() reads it.
  ...
  function netLatest() {
    try {
      const n = document.getElementById("vs-qwen-net");
  ...
  // Walk element text, skipping .vs-chip and any excluded selector.
  ...
  // Scope to the site's composer only; skip VoidScript's own injected textarea
  // (#vs-set-text inside #vs-root) so login pages without a site editor return
  ...
  // keep #vs-bar in #vs-root (position:fixed) and hug it to the composer's top
  ...
  function setInputLock(on) {
    ...
    if (on) {
      ...
      ed.setAttribute("data-vs-locked", "1");
    } else {
      ed.removeAttribute("readonly");
      ed.removeAttribute("data-vs-locked");
  ...
  function streamText(item) {
    ...
    const bd = bodyEl(item);
    return bd ? textWithout(bd, ".vs-chip") : "";
  }
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…VoidScript: result truncated to fit Qwen's ${SEND_CAP}-character input ` +
  ...
  async function typeAndSend(text, images) {
    ...
    const wasLocked = !!ed.getAttribute("data-vs-locked");
  ...
  // .vs-cmd-mask and let the CSS rule (overlay.css) re-hide recreated pre
  ...
  function findToolBlockSpot(item) {
    ...
    [...bd.children].forEach((el) => {
      if (el.classList.contains("vs-chip") || el.closest(S.codeWrap) || el.querySelector(S.codeWrap)) return;
  ...
  // build onto <html data-vs-qwen-ver>; read it from the page to confirm which
  ...
  function setVersionBeacon() {
    try { document.documentElement.setAttribute("data-vs-qwen-ver", QWEN_VER); } catch {}
  }
  ...
  // tool-calls instead of VoidScript commands. Removed 2026-07: the real cause of
  ...
  const MODEL_VIS_LS = "vsQwenModelVision2"; // bumped: old key may hold a stale Max-Preview=false
