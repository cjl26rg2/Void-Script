...
```
core/config.js        system prompt, feedback strings, tool categories (global VS)
core/parser.js        VoidScript command parsing - pure string logic   (global VSParse)
core/main.js          agentic loop, UI, camouflage, session state      (uses VSProvider)
providers/deepseek.js everything DeepSeek-specific: DOM selectors, generation
                      detection, send mechanics, composer modes…       (global VSProvider)
providers/gemini.js   same interface for Google Gemini (Angular DOM, Quill
                      composer, code-block masking)                    (global VSProvider)
providers/kimi.js     same interface for Kimi / Moonshot AI (Vue DOM, Lexical
                      composer, segment-code masking)                  (global VSProvider)
providers/glm.js      same interface for GLM / Z.ai (Svelte DOM, code-block
                      wrapper masking)                                 (global VSProvider)
providers/qwen.js     same interface for Qwen / chat.qwen.ai (Vue DOM, network-tap
                      SSE stream, Monaco disposal guard)               (global VSProvider)
...
providers/arena.js    same interface for Arena / arena.ai (React DOM, multi-model
                      playground, A/B-comparison auto-commit, Direct-mode gate;
                      /agent route is routed to a BETA VSGeneric provider)
                      (global VSProvider)
providers/meta.js     same interface for Meta AI / meta.ai (React DOM, textarea
                      composer, JSON-viewer + code-collapse masking)   (global VSProvider)
providers/_generic.js selector-driven factory VSGeneric(cfg) implementing the whole
                      VSProvider interface with framework-neutral defaults (beta base)
providers/chatgpt.js  BETA: thin config on VSGeneric for ChatGPT      (global VSProvider)
providers/grok.js     BETA: thin config on VSGeneric for Grok         (global VSProvider)
providers/perplexity.js BETA: thin config on VSGeneric for Perplexity (global VSProvider)
providers/copilot.js  BETA: thin config on VSGeneric for Copilot      (global VSProvider)
providers/mistral.js  BETA: thin config on VSGeneric for Mistral      (global VSProvider)
...
providers/manus.js, together.js
                      BETA: thin VSGeneric configs for more chat sites (global VSProvider)
...
`VSProvider` interface. There are two ways to integrate another AI site:

- **Quick (beta):** add a thin config file that calls `VSGeneric({ id, displayName,
